...
function RadioPrograms() {
  ...
  const radioPrograms = [
    {
      ...
      image: "/src/images/Radio Cover.jpeg",
      duration: "2 שעות",
    },
    {
      id: 2,
  ...
  const prevRadio = () => {
    setCurrentRadioIndex(
      (prev) => (prev - 1 + radioPrograms.length) % radioPrograms.length
    );
  };
    ...
    <div className="media-page">
      <div className="media-hero">
        <div className="media-hero-content">
          ...
          <p className="media-hero-subtitle">
            האזינו לתוכניות הרדיו המובילות שלנו
          </p>
        </div>
      </div>
      ...
      <div className="media-content-section radio-section">
        ...
        <div className="carousel-container">
          <button className="carousel-btn prev" onClick={prevRadio}>
            ❮
          </button>
          <div className="carousel-content">
            <div className="radio-card">
              <div className="radio-cover-image">
                <img
                  src="/src/images/RADIOCOVER.jpeg"
                  alt="Radio Cover"
                  className="radio-cover"
                />
              </div>
              <h3 className="radio-title">
                {radioPrograms[currentRadioIndex].title}
              </h3>
              <p className="radio-description">
                {radioPrograms[currentRadioIndex].description}
              </p>
              <div className="radio-duration">
                {radioPrograms[currentRadioIndex].duration}
              </div>
              <button className="listen-btn">האזן עכשיו</button>
            </div>
          </div>
          <button className="carousel-btn next" onClick={nextRadio}>
            ❯
          </button>
        </div>
        <div className="carousel-indicators">
          {radioPrograms.map((_, index) => (
            <span
              key={index}
              className={`indicator ${
                index === currentRadioIndex ? "active" : ""
              }`}
